% nit_eegplot() - Scroll (horizontally and/or vertically) through multichannel data.
%             Allows vertical scrolling through channels and manual marking 
%             and unmarking of data stretches or epochs for rejection.
% Usage: 
%           >> nit_eegplot(data, 'key1', value1 ...); % use interface buttons, etc.
%      else
%           >> nit_eegplot('noui', data, 'key1', value1 ...); % no user interface;
%                                                         % use for plotting
% Menu items:
...
%                    keeping track of several simultaneous nit_eegplot() windows). 
...
% Edit by Li Dong (Li_dong729@163.com), UESTC.
function [outvar1] = nit_eegplot(data, varargin); % p1,p2,p3,p4,p5,p6,p7,p8,p9)

% Defaults (can be re-defined):

DEFAULT_PLOT_COLOR = { [0 0 0], [1 1 1]};         % EEG line color
try, icadefs;
	DEFAULT_FIG_COLOR = BACKCOLOR;
	BUTTON_COLOR = GUIBUTTONCOLOR;
catch
	DEFAULT_FIG_COLOR = [1 1 1];
	BUTTON_COLOR =[0.86 0.86 0.86];
end;
...
if ~isstr(data) % If NOT a 'noui' call or a callback from uicontrols
   ...
   %catch
   %		disp('nit_eegplot() error: calling convention {''key'', value, ... } error'); return;
   %end;	
  ...
  % ---------------------------------
  defdowncom   = 'nit_eegplot(''defdowncom'',   gcbf);'; % push button: create/remove window
  defmotioncom = 'nit_eegplot(''defmotioncom'', gcbf);'; % motion button: move windows or display current position
  defupcom     = 'nit_eegplot(''defupcom'',     gcbf);';
  defctrldowncom = 'nit_eegplot(''topoplot'',   gcbf);'; % CTRL press and motion -> do nothing by default
  ...
  defctrlupcom = ''; % CTRL press and up -> do nothing by default
   ...
   try, g.title; 		    catch, g.title		= ['Scroll activity -- nit_eegplot()']; 	end;
   ...
   try, g.wincolor; 		catch, g.wincolor   = [ 0.9 0.9 0.9]; end; % change color edit by Li Dong $ 7/11/2015
   ...
   % ----------------------------
   % add other items
   % (added by Li Dong 2015/8/1)
   try, g.EventMarkerColor;  catch, g.EventMarkerColor = [1,0.4,0.4];end; % temp marker color
   try, g.eventlatencyend;  catch, g.eventlatencyend = [];end; % eventlatencyend
   try, g.eventlatencies;  catch, g.eventlatencies = [];end; % eventlatencies
   try, g.eventtypes;  catch, g.eventtypes = [];end; % eventtypes
   try, g.eventtypecolors;  catch, g.eventtypecolors = [];end; % eventtypecolors
   try, g.eventtypestyle;  catch, g.eventtypestyle = [];end; % eventtypestyle
   try, g.eventtypewidths;  catch, g.eventtypewidths = [];end; % eventtypewidths
   % -----------------------------
   ...
   for index=1:length(gfields)
      ...
      case {'spacing', 'srate' 'eloc_file' 'winlength' 'position' 'title' ...
               ...
               'maxeventstring' 'ploteventdur' 'butlabel' 'scale' 'events'...
               'data2' 'plotdata2' 'mocap' 'selectcommand' 'ctrlselectcommand'...
               'datastd' 'normed' 'envelope' 'EventMarkerColor' 'eventlatencyend'...
               'eventlatencies' 'eventtypes' 'eventtypecolors' 'eventtypestyle'...
               'eventtypewidths' }, % add other items, edit by Li Dong 2015/7/31 
      end;
   end;
  ...
  try chanlocs = evalin('base','tempChanlocs.chanlocs');
      YLabels = { chanlocs.labels };
      YLabels = char(YLabels);
  catch
      try chanlocs = evalin('base','EEG.chanlocs');
          YLabels = { chanlocs.labels };
          YLabels = char(YLabels);
      catch
  ...
  if isstr(g.eloc_file) | isstruct(g.eloc_file)  % Read in electrode names
      ...
      nit_eegplot('setelect', g.eloc_file, ax1);
  end;
  
  % Plot arrow added by Li Dong (2016/3/8)
  annotation(figh,'textarrow',[0.97, 0.97],[0.03, 0.08],'string','Neg'); % negative arrow
...
% positions of buttons
  ...
  posbut(12,:) = [ 0.59    0.02    0.06    0.05 ]; % REJECT Edit by Li Dong 2015/7/31
  posbut(13,:) = [-0.15   0.02    0.07    0.05 ]; % cancel
  posbut(17,:) = [-0.06    0.02    0.09    0.05 ]; % events types
  posbut(20,:) = [0.76   0.45     0.015    0.4 ]; % slider % edit by Li Dong 2015/7/17
  posbut(21,:) = [0.738    0.87    0.06      0.048];% normalize
  posbut(22,:) = [0.738    0.93    0.06      0.048];% stack channels(same offset)
  % --------------------------
  % edit by Li Dong 2015/7/24
  posbut(23,:) = [-0.195    0.3    0.05      0.6]; % event Pop-up Menu 
  posbut(24,:) = [-0.195    0.2    0.05      0.65];  % event listbox 
  posbut(25,:) = [-0.195    0.9    0.06      0.04]; % event text
  posbut(26,:) = [-0.195    0.1    0.05      0.04]; % event delete button 
  posbut(27,:) = [ 0.68    0.02    0.06    0.05 ]; % SAVE button
...
u(22) = uicontrol('Parent',figh, ...
  ...
  if ~isempty(g.events)
      u(17) = uicontrol('Parent',figh, ...
                        'Units', 'normalized', ...
                        'Position',posbut(17,:), ...
                        'string', 'Event types', 'callback', 'nit_eegplot(''drawlegend'', gcbf)');
  end;

  % -----------------
  % Event Pop Menu and listBox
  % edit by Li Dong, 2015/7/24
  if ~isempty(g.events)
      if isstr(g.events(1).type)
          [g.eventtypes,~,~] = unique_bc({g.events.type}); % indexcolor countinas the event type
      else [g.eventtypes, tmpind,~] = unique_bc([ g.events.type ]);
      end;
      
      cb_EventListbox = ['g = get(gcbf, ''userdata'');' ... 
          'OBJ_listbox = findobj(gcf,''Tag'',''EventListbox'');'...
          'strs_listbox = cellstr(get(OBJ_listbox,''String''));'...
          'value_select = get(OBJ_listbox,''Value'');'...
          't1 = str2num(strs_listbox{value_select,1});'...
          'OBJ_EPosition = findobj(''Tag'',''EPosition'');'...
          'set(OBJ_EPosition,''string'',num2str(t1));'...
          'g.time = t1;'...
          'set(findobj(gcf,''Tag'',''EEGPLOT''), ''userdata'', g);'...
          'nit_eegplot(''drawp'',0);'...
           ];
      ...
      u(24) = uicontrol('Parent',figh, ...
          ...
          'Visible','off',...
          'Tag','EventListbox',...
          'string','', 'callback', cb_EventListbox);
      
      cb_EventPopupMenu = ['g = get(gcbf, ''userdata'');' ... 
          'OBJ_EventPopMenu = findobj(gcf,''Tag'',''EventPopMenu'');'...
          'str1 = cellstr(get(OBJ_EventPopMenu,''String''));'...
          'value1 = get(OBJ_EventPopMenu,''Value'');'...
          'k = 1;'...
          'SelectEventLatencies = [];'...
          'for i = 1:length(g.events)'...
              'if strcmp(num2str(g.events(1,i).type),str1{value1,1})'...
              'SelectEventLatencies(k,1) = g.events(1,i).latency;'...
              'k = k + 1;'...
              'end;'...
          'end;'...
          'OBJ_listbox = findobj(gcf,''Tag'',''EventListbox'');'...
          'set(OBJ_listbox,''String'',num2str(SelectEventLatencies ./ g.srate),''Value'',1);'...
          'set(OBJ_listbox,''Visible'',''on'');'...
          ];
      u(23) = uicontrol('Parent',figh, ...
          ...
          'Style','popupmenu',...
          'Tag','EventPopMenu',...
          'string',g.eventtypes, 'callback', cb_EventPopupMenu);
      u(25) = uicontrol('Parent',figh, ...
          ...
          'Tag','text_EventPopMenu');
      
      cb_DeleteEvent = ['g = get(gcbf, ''userdata'');' ...
          'OBJ_listbox = findobj(gcf,''Tag'',''EventListbox'');'...
          'strs_listbox = cellstr(get(OBJ_listbox,''String''));'...
          'value_select = get(OBJ_listbox,''Value'');'...
          't1 = str2num(strs_listbox{value_select,1});'...
          'OBJ_EventPopMenu = findobj(gcf,''Tag'',''EventPopMenu'');'...
          'str_EventType = cellstr(get(OBJ_EventPopMenu,''String''));'...
          'value_EventType = get(OBJ_EventPopMenu,''Value'');'...
          'EventType = str_EventType{value1,1};'...
          'i = 1;'...
          'while strcmp(num2str(g.events(1,i).type),EventType)*(g.events(1,i).latency / g.srate == t1) ~=1;'...
              'i = i+1;'...
          'end;'...
          'g.events(i) = [];'...
          'g.eventwidths(i) = [];'...
          ...
          'g.eventcolors(i) = [];'...
          'g.eventstyle(i) = [];'...
          'strs_listbox(value_select) = [];'...
          'set(findobj(gcf,''Tag'',''EEGPLOT''), ''userdata'', g);'...
          'set(OBJ_listbox,''String'',strs_listbox,''Value'',1);'...
          'nit_eegplot(''drawp'',0);'...
          ];
      ...
      u(26) = uicontrol('Parent',figh, ...
          ...
          'Style','pushbutton',...
          'Visible','on',...
          'Tag','pushbutton_DeleteEvent',...
          'FontSize',8,...
          'String','Delete Event',...
          'callback', cb_DeleteEvent);
  end
  % -------------------------------------
  % Edit by Li Dong $ 2015/7/31
  % Right-click menu for adding events
  h_EventsMenu = uicontextmenu;

  cb_Type1 = ['g = get(gcbf, ''userdata'');' ...
      'OBJ_Etime = findobj(''Tag'',''Etime'');'...
      'TimeStr = get(OBJ_Etime(1),''string'');'...
      'currentTime = str2num(TimeStr);'...
      'if ~isempty(g.winrej)'...
          'CurrTimePoints = round(currentTime * g.srate);'...
          'RowsIX = find(g.winrej(:,1)<= CurrTimePoints & g.winrej(:,2)>= CurrTimePoints);'...
          'if ~isempty(RowsIX)'...
              'k = length(g.events)+1;'...
              'if ~ischar(g.events(1,1).type)'...
                  'g.events(1,k).type = 1001;'...
              'end;'...
              'if ischar(g.events(1,1).type)'...
                  'g.events(1,k).type = ''1001'';'...
              'end;'...
              'g.events(1,k).latency = round(g.winrej(RowsIX(1),1));'...
              'g.events(1,k).urevent = 1;'...
              'g.events(1,k).duration = round(g.winrej(RowsIX(1),2) - g.winrej(RowsIX(1),1));'...
              'g.eventcolors{1,k} = g.EventMarkerColor;'...
              'g.eventstyle{1,k} = ''-'';'...
              'g.eventwidths(1,k) = 1;'...
              'if ~isempty(g.eventlatencyend)'...
                  'g.eventlatencyend(1,k) = round(g.winrej(RowsIX(1),2));'...
              'end;'...
              'g.eventlatencies = [g.eventlatencies,round(g.winrej(RowsIX(1),1))];'...
              'try LogiVal = g.eventtypes == 1001;'...
              'catch, LogiVal = cellfun(@(x) isequal(x,''1001''), g.eventtypes);'...
              'end;'...
              'if sum(LogiVal) == 0'...
                  'g.eventtypecolors = [g.eventtypecolors,g.EventMarkerColor];'...
                  'g.eventtypestyle = [g.eventtypestyle,''-''];'...
                  'g.eventtypewidths = [g.eventtypewidths,1];'...
                  'if ~ischar(g.events(1,1).type)'...
                      'g.eventtypes = [g.eventtypes,1001];'...
                  'end;'...
                  'if ischar(g.events(1,1).type)'...
                      'g.eventtypes = [g.eventtypes,''1001''];'...
                  'end;'...
              'end;'...
              'set(findobj(gcf,''Tag'',''EEGPLOT''), ''userdata'', g);'...
              'set(findobj(gcf,''Tag'',''EventPopMenu''), ''string'', g.eventtypes);'...
              'nit_eegplot(''drawp'',0);'...
          'end;'...
      'elseif isempty(g.winrej)'...
          'CurrTimePoints = round(currentTime * g.srate);'...
          'k = length(g.events)+1;'...
          'if ~ischar(g.events(1,1).type)'...
              'g.events(1,k).type = 1001;'...
          ...
          'if ischar(g.events(1,1).type)'...
              'g.events(1,k).type = ''1001'';'...
          'end;'...
          'g.events(1,k).latency = CurrTimePoints;'...
          'g.events(1,k).urevent = 1;'...
          'g.events(1,k).duration = 1;'...
          'g.eventcolors{1,k} = ''k'';'...
          'g.eventstyle{1,k} = ''-'';'...
          'g.eventwidths(1,k) = 1;'...
          'g.eventlatencies = [g.eventlatencies,CurrTimePoints];'...
          'if ~isempty(g.eventlatencyend)'...
              'g.eventlatencyend(1,k) = CurrTimePoints+1;'...
          'end;'...
          'try LogiVal = g.eventtypes == 1001;'...
          'catch, LogiVal = cellfun(@(x) isequal(x,''1001''), g.eventtypes);'...
          'end;'...
          'if sum(LogiVal) == 0'...
              'g.eventtypecolors = [g.eventtypecolors,''k''];'...
              'g.eventtypestyle = [g.eventtypestyle,''-''];'...
              'g.eventtypewidths = [g.eventtypewidths,1];'...
              'if ~ischar(g.events(1,1).type)'...
                  'g.eventtypes = [g.eventtypes,1001];'...
  ...
  cb_Type2 = ['g = get(gcbf, ''userdata'');' ...
      ...
      'if ~isempty(g.winrej)'...
          ...
          'if ~isempty(RowsIX)'...
              ...
              'end;'...
              'g.eventlatencies = [g.eventlatencies,round(g.winrej(RowsIX(1),1))];'...
              'try LogiVal = g.eventtypes == 1002;'...
              'catch, LogiVal = cellfun(@(x) isequal(x,''1002''), g.eventtypes);'...
              'end;'...
              'if sum(LogiVal) == 0'...
                  'g.eventtypecolors = [g.eventtypecolors,g.EventMarkerColor];'...
                  'g.eventtypestyle = [g.eventtypestyle,''-''];'...
                  'g.eventtypewidths = [g.eventtypewidths,1];'...
                  'if ~ischar(g.events(1,1).type)'...
                      'g.eventtypes = [g.eventtypes,1002];'...
                  'end;'...
                  'if ischar(g.events(1,1).type)'...
                      'g.eventtypes = [g.eventtypes,''1002''];'...
                  'end;'...
  ...
  cb_Type3 = ['g = get(gcbf, ''userdata'');' ...
      ...
      'if ~isempty(g.winrej)'...
          ...
          'if ~isempty(RowsIX)'...
              ...
              'end;'...
              'g.eventlatencies = [g.eventlatencies,round(g.winrej(RowsIX(1),1))];'...
              'try LogiVal = g.eventtypes == 1003;'...
              'catch, LogiVal = cellfun(@(x) isequal(x,''1003''), g.eventtypes);'...
              'end;'...
              'if sum(LogiVal) == 0'...
                  'g.eventtypecolors = [g.eventtypecolors,g.EventMarkerColor];'...
                  'g.eventtypestyle = [g.eventtypestyle,''-''];'...
                  'g.eventtypewidths = [g.eventtypewidths,1];'...
                  'if ~ischar(g.events(1,1).type)'...
                      'g.eventtypes = [g.eventtypes,1003];'...
                  'end;'...
                  'if ischar(g.events(1,1).type)'...
                      'g.eventtypes = [g.eventtypes,''1003''];'...
                  'end;'...
  ...
  cb_Type4 = ['g = get(gcbf, ''userdata'');' ...
      ...
      'if ~isempty(g.winrej)'...
          ...
          'if ~isempty(RowsIX)'...
              ...
              'end;'...
              'g.eventlatencies = [g.eventlatencies,round(g.winrej(RowsIX(1),1))];'...
              'try LogiVal = g.eventtypes == 1004;'...
              'catch, LogiVal = cellfun(@(x) isequal(x,''1004''), g.eventtypes);'...
              'end;'...
              'if sum(LogiVal) == 0'...
                  'g.eventtypecolors = [g.eventtypecolors,g.EventMarkerColor];'...
                  'g.eventtypestyle = [g.eventtypestyle,''-''];'...
                  'g.eventtypewidths = [g.eventtypewidths,1];'...
                  'if ~ischar(g.events(1,1).type)'...
                      'g.eventtypes = [g.eventtypes,1004];'...
                  'end;'...
                  'if ischar(g.events(1,1).type)'...
                      'g.eventtypes = [g.eventtypes,''1004''];'...
                  'end;'...
  ...
  cb_Type5 = ['g = get(gcbf, ''userdata'');' ...
      ...
      'if ~isempty(g.winrej)'...
          ...
          'if ~isempty(RowsIX)'...
              ...
              'end;'...
              'g.eventlatencies = [g.eventlatencies,round(g.winrej(RowsIX(1),1))];'...
              'try LogiVal = g.eventtypes == 1005;'...
              'catch, LogiVal = cellfun(@(x) isequal(x,''1005''), g.eventtypes);'...
              'end;'...
              'if sum(LogiVal) == 0'...
                  'g.eventtypecolors = [g.eventtypecolors,g.EventMarkerColor];'...
                  'g.eventtypestyle = [g.eventtypestyle,''-''];'...
                  'g.eventtypewidths = [g.eventtypewidths,1];'...
                  'if ~ischar(g.events(1,1).type)'...
                      'g.eventtypes = [g.eventtypes,1005];'...
                  'end;'...
                  'if ischar(g.events(1,1).type)'...
                      'g.eventtypes = [g.eventtypes,''1005''];'...
                  'end;'...
  ...
  cb_BadBlock = ['g = get(gcbf, ''userdata'');' ...
      'OBJ_Etime = findobj(''Tag'',''Etime'');'...
      'TimeStr = get(OBJ_Etime(1),''string'');'...
      'currentTime = str2num(TimeStr);'...
      'if ~isempty(g.winrej)'...
          ...
          'end;'...
          'g.eventlatencies = [g.eventlatencies,round(g.winrej(RowsIX(1),1))];'...
          'try LogiVal = g.eventtypes == 9999;'...
          'catch, LogiVal = cellfun(@(x) isequal(x,''9999''), g.eventtypes);'...
          'end;'...
          'if sum(LogiVal) == 0'...
              'g.eventtypecolors = [g.eventtypecolors,g.EventMarkerColor];'...
              'g.eventtypestyle = [g.eventtypestyle,''-''];'...
              'g.eventtypewidths = [g.eventtypewidths,1];'...
              'if ~ischar(g.events(1,1).type)'...
                  'g.eventtypes = [g.eventtypes,9999];'...
              'end;'...
              'if ischar(g.events(1,1).type)'...
                  'g.eventtypes = [g.eventtypes,''9999''];'...
              'end;'...
  ...
  uimenu(h_EventsMenu,'Label','Type1','callback',cb_Type1);
  uimenu(h_EventsMenu,'Label','Type2','callback',cb_Type2);
  uimenu(h_EventsMenu,'Label','Type3','callback',cb_Type3);
  uimenu(h_EventsMenu,'Label','Type4','callback',cb_Type4);
  uimenu(h_EventsMenu,'Label','Type5','callback',cb_Type5);
  uimenu(h_EventsMenu,'Label','BadBlock','callback',cb_BadBlock);
  
  set(ax1,'Uicontextmenu',h_EventsMenu);
  % ------------------------
  % edit by Li Dong $ 2015/7/31
  % add SAVE button (save marked data)
  tmpcprint = 'fprintf(''Save the markers to EEG in the workspace!!!'')';
  cb_SAVE = [ 'g = get(gcbf, ''userdata'');' ...
              'try, EEG = evalin(''base'',''EEG'');'... 
                   'EEG.event = g.events;'...
                   'warndlg(''Events are only saved in the EEG in the workspace'',''Noting'');'...     
              ';catch, errordlg(''Saving Markers Failed!!! Please Check the EEG in Workspace!!!'');'...
              'end;']; % quitting expression 
          % tmpcprint...
  u(27) = uicontrol('Parent',figh, ...
				    ...
				    'Position',posbut(27,:), ...
					'Tag','pushbutton_SAVE',...
					'string','Save Events', 'callback',cb_SAVE);
  % ------------------------
  ...
  uimenu('Parent',m(7),'Label','Edit figure', 'Callback', 'nit_eegplot(''noui'');');
  ...
  % edit by Li Dong $ 2015/7/31
  cb_SaveImage = ['set(gcbf,''inverthardcopy'',''off'',''PaperUnits'', ''inch'');'...
                  'width = 8;'...
                  'height = 4.5;'...
                  'set(gcbf,''Paperposition'',[0 0 width height]);'...
                  'DPI = 300;'...
                 '[filename,pathname] = uiputfile({''*.JPG'',''JPEG'';'...
                 '''*.TIF'',''TIFF'';''*.BMP'',''BMP'';''*.EPS'',''EPS color'';'...
                 '''*.PNG'',''PNG''},''Save Image'');'...
                 'if ~isequal(filename,0) & ~isequal(pathname,0)'...
                     ...
                     'switch ext;'...
                           'case ''.PNG'';'...
                                'print(gcbf,fpath,''-dpng'',[''-r'',num2str(DPI)]);'...
                           'case ''.TIF'';'...
                                'print(gcbf,fpath,''-dtiff'',[''-r'',num2str(DPI)]);'...
                           'case ''.JPG'';'...
                                'print(gcbf,fpath,''-djpeg'',[''-r'',num2str(DPI)]);'...
                           'case ''.BMP'';'...
                                'print(gcbf,fpath,''-dbmp'',[''-r'',num2str(DPI)]);'...
                           'case ''.EPS'';'...
                                'print(gcbf,fpath,''-depsc'',[''-r'',num2str(DPI)]);'...
                     'end;'...
                     'msgbox(''Image has saved!'',''Success'',''help'');'...
              'end;'];
  uimenu('Parent',m(7),'Label','Save Image', 'Callback',cb_SaveImage);
  % ----------------------
  % Portrait %%%%%%%%
  ...
  % Scale Eye %%%%%%%%%
  timestring = ['[OBJ1,FIG1] = gcbo;',...
	        'nit_eegplot(''scaleeye'',OBJ1,FIG1);',...
		'clear OBJ1 FIG1;'];
  m(7) = uimenu('Parent',m(1),'Label','Show scale','Callback',timestring);
  
  % Title %%%%%%%%%%%%
  uimenu('Parent',m(1),'Label','Title','Callback','nit_eegplot(''title'')')
  ...
  uimenu('Parent',m(2),'Label','Time range to display',...
      'Callback','nit_eegplot(''window'')')
  
  % Electrode window %%%%%%%%
  uimenu('Parent',m(2),'Label','Number of channels to display',...
      'Callback','nit_eegplot(''winelec'')')
  ...
  uimenu('Parent',m(6),'Label','Load .loc(s) file',...
      'Callback','nit_eegplot(''loadelect'');')
  
  % Zooms %%%%%%%%
  zm = uimenu('Parent',m(2),'Label','Zoom off/on');
  commandzoom = [ 'set(gcbf, ''windowbuttondownfcn'', [ ''zoom(gcbf,''''down''''); nit_eegplot(''''zoom'''', gcbf, 1);'' ]);' ...
                  'tmpg = get(gcbf, ''userdata'');' ...
                  'set(gcbf, ''windowbuttonmotionfcn'', tmpg.commandselect{2}); clear tmpg tmpstr;'...
                  'tempAxisOBJ = findobj(''tag'',''eegaxis'');'...
                  'S = get(tempAxisOBJ);'...
                  'tempUIContextMenu = S.UIContextMenu;'...
                  'set(tempAxisOBJ,''UIContextMenu'',[]);']; %  make UIContextMenu unavailable when Zoom on (Li Dong 2015/8/1)
  ...
  uimenu('Parent',zm,'Label','Zoom off', 'separator', 'on', 'callback', ...
     ...
     'clear tmpg;'... 
     'tempAxisOBJ = findobj(''tag'',''eegaxis'');'...
     'set(tempAxisOBJ,''UIContextMenu'',tempUIContextMenu)']); %  make UIContextMenu available when Zoom off (Li Dong 2015/8/1)
  uimenu('Parent',figh,'Label', 'Help', 'callback', 'pophelp(''nit_eegplot'');');

  % Events %%%%%%%%
  zm = uimenu('Parent',m(2),'Label','Events');
  complotevent = [ 'tmpg = get(gcbf, ''userdata'');' ...
                  'tmpg.plotevent = ''on'';' ...                  
                  'set(gcbf, ''userdata'', tmpg); clear tmpg; nit_eegplot(''drawp'', 0);'];
  comnoevent   = [ 'tmpg = get(gcbf, ''userdata'');' ...
                  'tmpg.plotevent = ''off'';' ...                  
                  'set(gcbf, ''userdata'', tmpg); clear tmpg; nit_eegplot(''drawp'', 0);'];
  comeventmaxstring   = [ 'tmpg = get(gcbf, ''userdata'');' ...
                'tmpg.plotevent = ''on'';' ...
                'set(gcbf, ''userdata'', tmpg); clear tmpg; nit_eegplot(''emaxstring'');']; % JavierLC      
  comeventleg  = [ 'nit_eegplot(''drawlegend'', gcbf);'];
...
%  set(figh, 'busyaction', 'cancel');
  ...
  if ~isempty(g.events)
      ...
      g.eventcolors     = { [1,0.3,0.3], 'm', 'c', [0,0.6,1], [0,0.8,0], [0,0.75,0.75], [0.75, 0.75, 0], ...
                           [0.04, 0.52,0.78], [0.87,0.49,0], [1,0.6,0.78],[0.85, 0.7, 1], [1,0.69,0.39]}; % edit by Li Dong $ 2015/8/1
      % g.eventcolors     = { 'r', [0 0.8 0], 'm', 'c', 'k', 'b', [0 0.8 0] };% original code
      g.eventstyle      = { '-' '-' '-'  '-'  '-' '-' '-' '--' '--' '--'  '--' '--' '--' '--'};
      g.eventwidths     = [ 2.5 1 ];
  ...
  nit_eegplot('scaleeye', [], gcf);
  if strcmp(lower(g.scale), 'off')
	  nit_eegplot('scaleeye', 'off', gcf);
  end;
  
  nit_eegplot('drawp', 0);
  nit_eegplot('drawp', 0);
  if g.dispchans ~= g.chans
  	   nit_eegplot('zoom', gcf);
  end;  
  nit_eegplot('scaleeye', [], gcf);
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  case 'drawp' % Redraw EEG and change position
    ...
    % this test help to couple nit_eegplot windows
    if exist('p3')
    	figh = p3;
    	figure(p3);
    else	
    ...
    % channels in the nit_eegplot figure window.
    for i = 1:g.chans
        ...
        if isfield(g, 'eloc_file') & ...
            ...
            plot(-(data(g.chans-i+1,lowlim:highlim) -meandata(g.chans-i+1))+i*g.spacing + (g.dispchans+1)*(oldspacing-g.spacing)/2 +g.elecoffset*(oldspacing-g.spacing), ...
                'color', tmpcolor, 'clipping','on')  % edit by Li Dong (2016/3/8): add '-', -(data-meandata).
            plot(1,mean(-(data(g.chans-i+1,lowlim:highlim) -meandata(g.chans-i+1))... % edit by Li Dong (2016/3/8): add '-', -(data-meandata).
                +i*g.spacing + (g.dispchans+1)*(oldspacing-g.spacing)/2 +g.elecoffset*(oldspacing-g.spacing),2),'<r','MarkerFaceColor','r','MarkerSize',6);
        end
...
%        keyboard;  
				...
				abscmax = min(abscmax, round(maxXlim(2)-1));	 
   				for i = 1:g.chans
                    if g.winrej(tpmi,g.chans-i+1+5)
                        plot(abscmin+1:abscmax+1,-(data(g.chans-i+1,abscmin+lowlim:abscmax+lowlim) ...% edit by Li Dong (2016/3/8): add '-', -(data-meandata).
    ...
    set(ax1, 'ylim',[g.elecoffset*g.spacing (g.elecoffset+g.dispchans+1)*g.spacing] );
	 ...
	 if g.children ~= 0
		...
		nit_eegplot( 'drawp', p1, p2, g.children);
		figure(figh);
	 end;	  
...
{ 'Number of channels to display:' } , 'Change number of channels to display', 1,  { num2str(g.dispchans) });
   ...
   set(gcf, 'UserData', g);
   nit_eegplot('updateslider', fig);
   nit_eegplot('drawp',0);	
   nit_eegplot('scaleeye', [], fig);
   return;
  ...
  case 'loadelect' % load channels
	...
	nit_eegplot('setelect',[ inputpath inputname ],AXH0);
	return;
  ...
  case 'noui'
   ...
   case 'zoom' % if zoom
      ...
      % ------------------------------
      if exist('p2') == 1
          set(gcbf, 'windowbuttondownfcn', [ 'zoom(gcbf,''down''); nit_eegplot(''zoom'', gcbf, 1);' ]);
  ...
  case 'defdowncom'
    ...
    if strcmp(get(fig, 'SelectionType'),'normal');
        ...
        if g.incallback ~= 1 % interception of nestest calls
            ...
            if (tmppos(1) >= 0) & (tmppos(1) <= highlim),
                ...
                set(fig,'UserData', g);
                nit_eegplot('drawp', 0);  % redraw background
            end;
        end;
    end;
      case 'defctrldowncom'
   ...
   otherwise
      error(['Error - invalid nit_eegplot() parameter: ',data])
  end
end
