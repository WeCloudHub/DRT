    ...
    if ~isempty(EEG.event) & isfield(EEG.event, 'type') & isstr(EEG.event(1).type)
        ...
        if isempty(boundaries)
            if ~usefft
                ...
                EEG.data = eegfiltfft( EEG.data, options{1:end-2}); % edit by Li Dong (2015/11/30): only need first 6 inputs in 'options'.
            end;
        else
            options{4} = 0;
                    ...
                    try
                        ...
                        if ~usefft
                            ...
                            EEG.data(:,boundaries(n)+1:boundaries(n+1)) = ...
                                eegfiltfft(EEG.data(:,boundaries(n)+1:boundaries(n+1)), options{1:end-2}); % edit by Li Dong (2015/11/30): only need first 6 inputs in 'options'.
                        end;
                    catch
